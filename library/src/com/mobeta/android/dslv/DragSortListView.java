...
public class DragSortListView extends ListView {
	...
	/**
	 * A listener that receives a callback when the floating View (or more precisely the originally dragged item) is removed by one of the provided gestures.
	 */
	...
	/**
	 * For each DragSortListView Listener interface implemented by <code>adapter</code>, this method calls the appropriate set*Listener method with <code>adapter</code> as the argument.
	 * 
	 * @param adapter
	...
	/**
	 * Get the shuffle edge for item at position when top of item is at y-coord top. Assumes that current item heights are consistent with current float view location and thus expanded positions and slide fraction. i.e. Should not be called between update of expanded positions/slide fraction and layoutChildren.
	 * 
	 * @param position
	 * @param top
	 * @param height
	 ...
	 * 
	 * @param remove
	 *            Remove the dragged item from the list. Calls a registered RemoveListener, if one exists. Otherwise, calls the DropListener, if one exists.
	 * 
	...
	/**
	 * Start a drag of item at <code>position</code> using the registered FloatViewManager. Calls through to {@link #startDrag(int,View,int,int,int)} after obtaining the floating View from the FloatViewManager.
	 * 
	 * @param position
	 *            Item to drag.
	 * @param dragFlags
	 ...
	 * @param deltaX
	 *            Offset in x of the touch coordinate from the left edge of the floating View (i.e. touch-x minus float View left).
	 * @param deltaY
	 *            Offset in y of the touch coordinate from the top edge of the floating View (i.e. touch-y minus float View top).
	 * 
	 * @return True if the drag was started, false otherwise. This <code>startDrag</code> will fail if we are not currently in a touch event, there is no registered FloatViewManager, or the FloatViewManager returns a null View.
	 */
	 ...
	 *            Offset in y of the touch coordinate from the top edge of the floating View (i.e. touch-y minus float View top).
	 * 
	 * @return True if the drag was started, false otherwise. This <code>startDrag</code> will fail if we are not currently in a touch event, <code>floatView</code> is null, or there is a drag in progress.
	 */
	...
	/**
	 * Interface for customization of the floating View appearance and dragging behavior. Implement your own and pass it to {@link #setFloatViewManager}. If your own is not passed, the default {@link SimpleFloatViewManager} implementation is used.
	 */
	public interface FloatViewManager {
		/**
		 * Return the floating View for item at <code>position</code>. DragSortListView will measure and layout this View for you, so feel free to just inflate it. You can help DSLV by setting some {@link ViewGroup.LayoutParams} on this View; otherwise it will set some for you (with a width of FILL_PARENT and a height of WRAP_CONTENT).
		 * 
		 * @param position
		 *            Position of item to drag (NOTE: <code>position</code> excludes header Views; thus, if you want to call {@link ListView#getChildAt(int)}, you will need to add {@link ListView#getHeaderViewsCount()} to the index).
		 * 
		 ...
		 *            The floating View.
		 * @param location
		 *            The location (top-left; relative to DSLV top-left) at which the float View would like to appear, given the current touch location and the offset provided in {@link DragSortListView#startDrag}.
		 * @param touch
	...
	/**
	 * Allows for easy toggling between a DragSortListView and a regular old ListView. If enabled, items are draggable, where the drag init mode determines how items are lifted (see {@link setDragInitMode(int)}). If disabled, items cannot be dragged.
	 * 
	 * @param enabled
	...
	/**
	 * This better reorder your ListAdapter! DragSortListView does not do this for you; doesn't make sense to. Make sure {@link BaseAdapter#notifyDataSetChanged()} or something like it is called in your implementation. Furthermore, if you have a choiceMode other than none and the ListAdapter does not return true for {@link ListAdapter#hasStableIds()}, you will need to call {@link #moveCheckState(int, int)} to move the check boxes along with the list items.
	 * 
	 * @param l
	 */
	...
	/**
	 * Your implementation of this has to reorder your ListAdapter! Make sure to call {@link BaseAdapter#notifyDataSetChanged()} or something like it in your implementation.
	 * 
	 * @author heycosmo
	 * 
	 */
	...
	/**
	 * Completely custom scroll speed profile. Default increases linearly with position and is constant in time. Create your own by implementing {@link DragSortListView.DragScrollProfile}.
	 * 
	 * @param ssp
	 */
	 ...
	 * <p>
	 * A word of warning about a "feature" in Android that you may run into when dealing with movable list items: for an adapter that <em>does</em> have stable IDs, ListView will attempt to locate each item based on its ID and move the check state from the item's old position to the new position — which is all fine and good (and removes the need for calling this function), except for the half-baked approach. Apparently to save time in the naive algorithm used, ListView will only search for an ID in the close neighborhood of the old position. If the user moves an item too far (specifically, more than 20 rows away), ListView will give up and just force the item to be unchecked. So if there is a reasonable chance that the user will move items more than 20 rows away from the original position,
	 * you may wish to use an adapter with unstable IDs and call this method manually instead.
	 * 
	 * @param from
	 * @param to
	 */
	...
	/**
	 * Interface for controlling scroll speed as a function of touch position and time. Use {@link DragSortListView#setDragScrollProfile(DragScrollProfile)} to set custom profile.
	 * 
	 * @author heycosmo
	 * 
	 */
