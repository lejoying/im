...
//                console.error("getData event: 2");
//                zk.exists();
//            } else if (event.type == 3) {
//                console.error("getData event: 3");
//                zk.getData();
//            } else if (event.type == 4) {
//                console.error("getData event: 4");
//                zk.getData();
//            } else {
//                console.error("getData Exception deal with ......");
//            }
        },
        function (error, data, stat) {
            if (error) {
...
zk.exists = function (accessKeyPool, flag) {
    client.exists('/accessKeyPool',
        function (event) {
            if (event.type == 1) {
                console.error("exists event: 1");
                zk.exists();
            } else if (event.type == 2) {
                console.error("exists event: 2");
                zk.exists();
            } else if (event.type == 3) {
                console.error("exists event: 3");
                zk.getData();
            } else if (event.type == 4) {
                console.error("exists event: 4");
                zk.getData();
            } else {
                console.error("exists Exception deal with ......");
            }
        }, function (error, stat) {
            if (error) {
                console.log(error.stack);
